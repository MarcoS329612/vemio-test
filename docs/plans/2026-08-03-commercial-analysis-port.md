...
- Produces: `economics.mean_promo_discount(frame) -> DataFrame[product_code, product_name, mean_promo_discount]` — unit-weighted mean of the `discount` field over promoted lines (`is_promo`) that are also price-usable (`usable_for_price`). Added after round-1 review: the weekly panel's `discount_depth` (bruto-vs-net) is blind to combo-level discounts that never reach `bruto` line by line (F-004), so it cannot answer the SKU-level comparison against `break_even_discount` — `discount` is the field the dictionary defines as promotional depth, and that is what stage 04 compares instead. The `usable_for_price` restriction was added after round-2 review: without it, negative-`discount` rows (an unexplained surcharge per F-004/Q6, 9,928 of 10,084 dataset-wide concentrated on one SKU) pulled that SKU's mean toward an implausibly shallow number. A null `discount` on an otherwise usable row is still treated as zero (not dropped) — a deliberate, disclosed choice, not an oversight.
