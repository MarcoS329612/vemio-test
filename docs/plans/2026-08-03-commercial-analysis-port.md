...
- Produces: `economics.mean_promo_discount(frame) -> DataFrame[product_code, product_name, mean_promo_discount]` — unit-weighted mean of the `discount` field over promoted lines (`is_promo`) with a trustworthy `discount` value. Added after round-1 review: the weekly panel's `discount_depth` (bruto-vs-net) is blind to combo-level discounts that never reach `bruto` line by line (F-004), so it cannot answer the SKU-level comparison against `break_even_discount` — `discount` is the field the dictionary defines as promotional depth, and that is what stage 04 compares instead. **Not `usable_for_price`** — round-3 review found that combined flag's `is_zero_amount` clause (right for price/quantity regression, irrelevant to discount validity) silently drops free-goods lines, and a 100%-discount giveaway is a legitimate, informative promotional-depth observation; for SKU 1283 that alone was a 14% relative distortion, larger than the negative-discount fix that preceded it. The mask is composed explicitly instead: `is_promo & ~is_zero_quantity & ~is_missing_money & ~is_negative_discount` — every `usable_for_price` component except `is_zero_amount`. A null `discount` on an otherwise valid row is still treated as zero (not dropped) — a deliberate, disclosed choice, not an oversight.
- Produces: `economics.null_discount_unit_share(frame) -> DataFrame[product_code, product_name, null_discount_unit_share]` — share (%) of each SKU's promoted-line units whose `discount` is null. Added after round-2 review so the null→0 imputation `mean_promo_discount` makes is disclosed in `reports/04_elasticity.md` rather than only discoverable in source.
...
In `scripts/04_elasticity.py`, build a table joining `economics.break_even_discount(rates)` to each SKU's `economics.mean_promo_discount(flagged)` (unit-weighted mean of `discount` over promoted lines with a trustworthy discount value — not `usable_for_price` wholesale, see the interface note above), with a `cushion = break_even_discount - mean_promo_discount` column (negative means already below cost) and `already_below_cost = cushion < 0`. The cushion column was added after round-3 review: a bare `already_below_cost` table left a near-miss `False` SKU (1875, cushion ≈ 0.007) reading as a comfortable no, so state the margin numerically for every SKU and flag in prose any SKU whose cushion is within two points of its break-even depth as a close call, not a clear one. The weekly panel's `discount_depth` (bruto-vs-net) is shown alongside it in the same table, labelled so it cannot be mistaken for the comparison measure (`bruto_proxy_discount_depth`), so the order-of-magnitude disagreement between the two is visible rather than asserted — round-1 review found the panel proxy is blind to combo discounts that never reach `bruto` line by line (F-004) and cannot be used for the flag itself. Also join `economics.null_discount_unit_share(flagged)` and state in prose which SKU's figure is materially exposed to the null→0 imputation and by how much — round-2 review found this needed disclosing in the report, not just in the docstring. Write the table into `reports/04_elasticity.md` under a heading "Break-even discount by SKU", with prose stating plainly why `mean_promo_discount` is the correct comparison. Any SKU flagged `True` is a standing loss, not an occasional one — call that out in the surrounding prose.
